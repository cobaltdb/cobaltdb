...
- **[verified — partially FIXED; remainder is policy] Audit write durability** — `pkg/audit/logger.go`. Write failures were already logged + tracked in `al.lastErr`, but there was no programmatic signal and `flushBatch` silently dropped the whole batch when `file == nil`.
  **Fix applied (2026-05-29):** added an atomic `failedWrites` counter incremented at every drop point (per-event write error, sync error, and the previously-silent `file == nil` early return) plus a `FailedWriteCount() int64` accessor for monitoring/alerting; closed the silent `file == nil` drop. Added `pkg/audit/failed_write_count_test.go`. **Still open (policy):** (b) **retry** on transient I/O errors and (c) a **fail-secure** mode where a failed audit write aborts the audited operation — (c) is a fail-open-vs-fail-secure product decision (availability vs. guaranteed auditability) and is left for the owner. `Log()` remains fire-and-forget at its ~10 call sites in `database.go`.
- **[corrected] Replication apply-callback panics are handled correctly** — `pkg/replication/replication.go:308-318, 1033, 1102`. The claim that `OnApply` panics let execution "continue as if success" is **wrong**: `callOnApply`/`callOnApplySnapshot` convert a panic into a returned `error`, and both callers check it and `return` **before** advancing `lastApplied` — so there is no LSN drift. Only the notification-only hooks `OnLag`/`OnDisconnect` (replication.go:320-334) swallow panics with `_ = recover()`, which is acceptable for fire-and-forget callbacks (logging the panic would be a marginal nicety, low priority).
...
2. ✅ **DONE** — Parallel worker panics now isolated (§3.3). §4 investigated: replication apply path is correct (false positive); audit failures are logged but lack a metric/retry (safe to add) — fail-secure is a policy decision.
...
7. **Partly DONE.**
   - ✅ **Write-path safety net strengthened (prerequisite for decomposition).** Added `pkg/catalog/buffered_constraints_test.go` driving the buffered/MVCC snapshot constraint paths that were barely covered even by the full suite: `checkUniqueConstraintsSnapshot` 5.9%→52.9%, `checkForeignKeyConstraintsSnapshot` 3.9%→60.8%, `processUpdateRowDataSnapshot` 18%→30.6%, `validateInsertRowSnapshot` 50%→61%.
   - **[lead — NEEDS investigation]** While writing those tests: a buffered (in-transaction) `UPDATE` that sets a `UNIQUE` column to a value already held by another row *in the same transaction* was **not** rejected at statement time. This may be a correctness gap in the buffered UPDATE path (uniqueness possibly only enforced at commit, or not at all for in-txn duplicates). The test deliberately does not assert this as correct — confirm the intended semantics and add enforcement/a test once decided.
   - ✅ **`parser.go` file split (mechanical, behavior-preserving).** Split the 3,672-line `parser.go` into `parser.go` (754 — `Parser` struct, `Parse`, dispatch, token/nav helpers), `parser_expression.go` (616), `parser_ddl.go` (1,293), and `parser_dml_select.go` (1,028) via an AST-based lossless split (no bytes/comments lost, no duplicate/missing decls — compiler-verified), then `goimports`+`gofmt`. Lean + full + downstream catalog test suites pass unchanged.
   - ⏳ **PLANNED — deliberately not rushed:** decompose `insertLocked`/`updateLocked` and extract the row-decode helper (§5). These are hot *write* paths with tightly-interwoven local state (column indices, undo logging, buffered writes, index updates, triggers all share mutable state inside the row loop), so a mistake means silent data corruption. Do this as a dedicated, reviewed pass — ideally after lifting the lean-suite coverage of the insert/update/delete paths first (§9 item 4). Suggested order: (a) extract `decodeVisibleRow` and migrate read paths under test; (b) extract `validateRowAgainstConstraints` (shared by insert/update); (c) split `insertLocked` into `validateInsert` / `buildRowWithConstraints` / `recordInsertUndo` / `applyIndexUpdatesForInsert`.
