...
- **Prepared-statement result metadata** (`mysql.go` `handleStmtPrepare`/`sendBinaryResultSetFromRows`):
  ...
  - **[known limitation]** Both the prepare and binary execute result column definitions are typed as `MySQLTypeVarString` for *every* column (via `buildColumnDefPacket`/`defaultMySQLColumnDefinition`), while the **text** path reports real types via `ColumnTypeHints`. Values still round-trip correctly because `buildBinaryRowPacket` length-encodes them as strings and the columns are declared as strings (internally consistent — verified by `TestMySQLGoSQLDriverCompatibility`). Full binary-protocol type fidelity (declare `MySQLTypeLong`/`Double`/… *and* encode values in native fixed-width/IEEE-754 form) is a larger, separate change — do both together or neither, since a type/encoding mismatch would corrupt binary-protocol reads.
