...
- **[verified — policy decision, not a bug] Audit write durability** — `pkg/audit/logger.go:299-317, 356-381`. Write failures are **not** silent: both the sync-fallback (when the async channel is full) and `flushBatch` log the error via the structured logger and record it in `al.lastErr` (exposed). What's missing is (a) a programmatic dropped/failed-event **counter + metric** for alerting, (b) **retry** on transient I/O errors, and (c) a **fail-secure** mode where a failed audit write aborts the audited operation. (a)+(b) are safe, non-breaking improvements worth doing. (c) is a fail-open-vs-fail-secure **product policy decision** (it trades availability for guaranteed auditability) and should not be changed unilaterally. `Log()` is intentionally fire-and-forget at its ~10 call sites in `database.go`.
