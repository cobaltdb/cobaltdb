...
import { ArrowRight, ChevronRight, Github, Play, Terminal } from "lucide-react";
...
function AnimatedCounter({
	...
	return (
		<div
			ref={ref}
			className="text-3xl md:text-4xl font-black tracking-tight text-cobalt-600 dark:text-cobalt-400"
		>
...
const demoScript: DemoLine[] = [
	{
		type: "prompt",
		text: "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, role TEXT);",
		delay: 0,
	},
	{ type: "info", text: "Query OK. 0 row(s) affected. (0.02ms)", delay: 600 },
	{
		type: "prompt",
		text: "INSERT INTO users VALUES (1,'Alice','admin'), (2,'Bob','user'), (3,'Carol','admin');",
		delay: 1200,
	},
	{ type: "info", text: "Query OK. 3 row(s) affected. (0.05ms)", delay: 1800 },
	{
		type: "prompt",
		text: "SELECT name, role, RANK() OVER (ORDER BY name) as rnk FROM users WHERE role = 'admin';",
		delay: 2600,
	},
	{
		type: "table",
...
function TypingText({
	...
	return (
		<>
			...
			{!done && (
				<span className="inline-block w-1.5 h-4 bg-cobalt-400 ml-0.5 animate-pulse align-middle" />
			)}
		</>
	);
}
...
function InteractiveTerminal() {
	...
	return (
		<div className="rounded-xl overflow-hidden border border-gray-200 dark:border-white/10 bg-[#0a0c12] shadow-2xl shadow-cobalt-950/10 dark:shadow-cobalt-950/30">
			...
			<div
				ref={containerRef}
				className="p-4 font-mono text-[13px] leading-6 min-h-[280px] max-h-[340px] overflow-y-auto"
			>
				{visibleLines.map((line, i) => {
					if (line.type === "prompt") {
						return (
							<div key={i} className="flex items-start gap-2 mb-1">
								<span className="text-cobalt-400 select-none shrink-0">
									cobaltdb&gt;
								</span>
					...
					if (line.type === "info") {
						return (
							<div
								key={i}
								className="text-gray-500 text-xs mb-2 pl-1 animate-fade-in"
							>
								{line.text}
							</div>
						);
					...
					if (line.type === "table" && line.columns && line.rows) {
						const cols = line.columns as string[];
						const rows = line.rows as string[][];
						const widths = cols.map((col, ci) =>
							Math.max(col.length, ...rows.map((r) => r[ci]?.length || 0)),
						);
						...
						return (
							<div
								key={i}
								className="text-xs mb-1 text-gray-300 animate-fade-in"
							>
...
export function HeroSection() {
	return (
		<section className="relative overflow-hidden hero-radial">
			...
			<div
				className="absolute bottom-0 left-1/4 w-[400px] h-[400px] bg-cobalt-600/5 dark:bg-cobalt-600/8 rounded-full blur-[100px] animate-glow-pulse"
				style={{ animationDelay: "1s" }}
			/>
			<div
				className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-cobalt-500/3 rounded-full blur-[120px] animate-glow-pulse"
			...
			<div className="container mx-auto px-4 relative pt-16 pb-20 lg:pt-28 lg:pb-28">
				...
				<div className="text-center max-w-4xl mx-auto mb-12">
					...
					<p className="text-lg md:text-xl text-muted-foreground mb-8 max-w-2xl mx-auto leading-relaxed animate-fade-in-up stagger-2">
						Embed in your Go app or deploy as a standalone MySQL-compatible
						server. Pure Go, zero dependencies, AES-256 encryption, ACID
						transactions, replication, and 5,800+ tests.
					</p>
					...
					<div className="flex flex-col sm:flex-row items-center justify-center gap-3 animate-fade-in-up stagger-3">
						<Link to="/playground">
							<Button
								size="lg"
								className="bg-cobalt-600 hover:bg-cobalt-700 text-white shadow-lg shadow-cobalt-600/25 hover:shadow-cobalt-600/40 transition-all text-base px-7 group"
							>
						...
						<Link to="/docs">
							<Button
								size="lg"
								variant="outline"
								className="text-base px-7 group"
							>
								Get Started
						...
						>
							<Button
								size="lg"
								variant="ghost"
								className="text-base px-5 group"
							>
				...
				<div className="flex flex-wrap items-center justify-center gap-3 mt-10 animate-fade-in-up stagger-5">
					...
					<span className="px-2.5 py-1 rounded-md bg-muted text-xs font-mono">
						mysql CLI
					</span>
					<span className="px-2.5 py-1 rounded-md bg-muted text-xs font-mono">
						go-sql-driver
				...
				<div className="grid grid-cols-4 gap-4 max-w-3xl mx-auto mt-10 animate-fade-in-up stagger-5">
					...
					<div className="text-center p-4 rounded-xl hover:bg-card/50 transition-colors">
						<AnimatedCounter target={22} suffix=" fixes" />
						<div className="text-sm text-muted-foreground mt-1 font-medium">
							Security v0.6
						</div>
					</div>
