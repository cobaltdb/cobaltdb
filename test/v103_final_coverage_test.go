...
func TestV103FinalCoverage(t *testing.T) {
	...
	t.Run("GroupByOrderBy", func(t *testing.T) {
		...
		t.Run("OrderBySumExprArg", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][0]) != "west" {
				t.Fatalf("expected west, got %v", rows[0][0])
			}
		})
		...
		t.Run("StringComparisonOrderBy", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][0]) != "east" {
				t.Fatalf("expected east, got %v", rows[0][0])
			}
		})
	...
	t.Run("JoinGroupByAggregates", func(t *testing.T) {
		...
		t.Run("COUNT_Column_Join", func(t *testing.T) {
			...
			for _, r := range rows {
				...
				case "Engineering":
					if fmt.Sprintf("%v", r[1]) != "2" {
						t.Fatalf("expected 2, got %v", r[1])
					}
				case "Sales":
					if fmt.Sprintf("%v", r[1]) != "1" {
						t.Fatalf("expected 1, got %v", r[1])
					}
				case "HR":
					if fmt.Sprintf("%v", r[1]) != "0" {
						t.Fatalf("expected 0, got %v", r[1])
					}
				}
			}
		})
	...
	t.Run("DerivedTable", func(t *testing.T) {
		...
		t.Run("SubqueryInFrom", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][0]) != "beta" {
				t.Fatalf("expected beta, got %v", rows[0][0])
			}
		})
		t.Run("DerivedTableAggregate", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][0]) != "60" {
				t.Fatalf("expected 60, got %v", rows[0][0])
			}
		})
	...
	t.Run("FunctionCalls", func(t *testing.T) {
		...
		t.Run("HEX_String", func(t *testing.T) {
			...
			if len(fmt.Sprintf("%v", rows[0][0])) == 0 {
				t.Fatal("empty")
			}
		})
		...
		t.Run("QUOTE_String", func(t *testing.T) {
			...
			if !strings.Contains(fmt.Sprintf("%v", rows[0][0]), "hello") {
				t.Fatal("bad QUOTE")
			}
		})
		...
		t.Run("PRINTF", func(t *testing.T) {
			...
			if got != "Alice has 5 items" {
				t.Fatalf("expected 'Alice has 5 items', got '%v'", got)
			}
		})
		t.Run("STRFTIME", func(t *testing.T) {
			...
			if !strings.Contains(got, "2024") {
				t.Fatalf("expected to contain 2024, got %v", got)
			}
		})
		t.Run("DATE", func(t *testing.T) {
			...
			if !strings.Contains(fmt.Sprintf("%v", rows[0][0]), "2024") {
				t.Fatal("bad DATE")
			}
		})
	})
	...
	t.Run("JSONFunctions", func(t *testing.T) {
		...
		t.Run("JSON_PRETTY", func(t *testing.T) {
			...
			if !strings.Contains(fmt.Sprintf("%v", rows[0][0]), "a") {
				t.Fatal("bad JSON_PRETTY")
			}
		})
		...
		t.Run("JSON_TYPE_Array", func(t *testing.T) {
			...
			if !strings.Contains(strings.ToLower(fmt.Sprintf("%v", rows[0][0])), "array") {
				t.Fatal("bad JSON_TYPE")
			}
		})
		t.Run("JSON_TYPE_Object", func(t *testing.T) {
			...
			if !strings.Contains(strings.ToLower(fmt.Sprintf("%v", rows[0][0])), "object") {
				t.Fatal("bad JSON_TYPE")
			}
		})
	...
	t.Run("CTE", func(t *testing.T) {
		...
		t.Run("RecursiveCTE", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[4][0]) != "5" {
				t.Fatalf("expected 5, got %v", rows[4][0])
			}
		})
		t.Run("CTEWithUnion", func(t *testing.T) {
			...
			if len(rows) != 4 {
				t.Fatalf("expected 4, got %d", len(rows))
			}
		})
	...
	t.Run("GroupByOrderByMultiAgg", func(t *testing.T) {
		...
		if fmt.Sprintf("%v", rows[0][0]) != "X" {
			t.Fatalf("expected X, got %v", rows[0][0])
		}
	})
	...
	t.Run("MoreFunctions", func(t *testing.T) {
		...
		t.Run("REPLACE_Func", func(t *testing.T) {
			afExpectVal(t, db, ctx, "SELECT REPLACE('hello world', 'world', 'there')", "hello there")
		})
	...
	t.Run("RecursiveCTEHierarchy", func(t *testing.T) {
		...
		if fmt.Sprintf("%v", rows[0][1]) != "root" {
			t.Fatalf("expected root, got %v", rows[0][1])
		}
		if fmt.Sprintf("%v", rows[3][2]) != "2" {
			t.Fatalf("expected level 2, got %v", rows[3][2])
		}
	})
	...
	t.Run("JoinGroupByOrderBy", func(t *testing.T) {
		...
		if fmt.Sprintf("%v", rows[0][0]) != "Electronics" {
			t.Fatalf("expected Electronics, got %v", rows[0][0])
		}
	...
	t.Run("WindowFunctionPaths", func(t *testing.T) {
		...
		t.Run("SUM_Window", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][2]) != "10" {
				t.Fatalf("expected 10, got %v", rows[0][2])
			}
			if fmt.Sprintf("%v", rows[1][2]) != "30" {
				t.Fatalf("expected 30, got %v", rows[1][2])
			}
		})
		t.Run("COUNT_Window", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][2]) != "2" {
				t.Fatalf("expected 2, got %v", rows[0][2])
			}
		})
		t.Run("ROW_NUMBER_Window", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][2]) != "1" {
				t.Fatalf("expected 1, got %v", rows[0][2])
			}
		})
	...
	t.Run("OrderByExprPaths", func(t *testing.T) {
		...
		if got0 != "50" {
			t.Fatalf("expected 50, got %v", got0)
		}
	})
	...
	t.Run("CaseBetweenIn", func(t *testing.T) {
		...
		t.Run("SearchedCASE", func(t *testing.T) {
			...
			if fmt.Sprintf("%v", rows[0][1]) != "A" {
				t.Fatalf("expected A, got %v", rows[0][1])
			}
			if fmt.Sprintf("%v", rows[3][1]) != "F" {
				t.Fatalf("expected F, got %v", rows[3][1])
			}
		})
	...
	t.Run("AlterTableRenameColumn", func(t *testing.T) {
		...
		if fmt.Sprintf("%v", rows[1][0]) != "second" {
			t.Fatalf("expected second, got %v", rows[1][0])
		}
	})
}
