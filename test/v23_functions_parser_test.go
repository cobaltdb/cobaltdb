...
func TestV23FunctionsAndParser(t *testing.T) {
	...
	checkRowCount("LIKE percent", "SELECT * FROM like_patterns WHERE val LIKE 'a%'", 2)     // abc, a%c (case-sensitive)
	checkRowCount("LIKE underscore", "SELECT * FROM like_patterns WHERE val LIKE '_bc'", 1) // abc (case-sensitive)
