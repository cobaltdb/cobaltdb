...
func TestComputeAggregatesCoverage(t *testing.T) {
	backend := storage.NewMemory()
	pool := storage.NewBufferPool(1024, backend)
	cat := New(nil, pool, nil)
	...
	for _, d := range data {
		_, _, err := cat.Insert(&query.InsertStmt{
			Table:   "test_compute_agg",
			Columns: []string{"id", "amount", "category"},
			Values:  [][]query.Expression{{&query.NumberLiteral{Value: float64(d.id)}, &query.NumberLiteral{Value: d.amount}, &query.StringLiteral{Value: d.category}}},
		}, nil)
		if err != nil {
			t.Fatalf("Failed to insert: %v", err)
		}
	...
	stmt := &query.SelectStmt{
		Columns: []query.Expression{
			...
			&query.FunctionCall{Name: "MIN", Args: []query.Expression{&query.Identifier{Name: "amount"}}},
			&query.FunctionCall{Name: "MAX", Args: []query.Expression{&query.Identifier{Name: "amount"}}},
		},
		From: &query.TableRef{Name: "test_compute_agg"},
	}
	...
	t.Logf("Aggregate results: cols=%v, rows=%v", cols, rows)
...
func TestComputeAggregatesWithNullColumn(t *testing.T) {
	backend := storage.NewMemory()
	pool := storage.NewBufferPool(1024, backend)
	cat := New(nil, pool, nil)
	...
	// We can't directly insert NULL with this API, but we test the code path
	_, _, err = cat.Insert(&query.InsertStmt{
		Table:   "test_agg_null_col",
	...
	stmt := &query.SelectStmt{
		...
		From: &query.TableRef{Name: "test_agg_null_col"},
	}
	...
	t.Logf("COUNT(column) result: %v", rows[0][0])
}
...
func TestComputeAggregatesNoGroupBy(t *testing.T) {
	backend := storage.NewMemory()
	pool := storage.NewBufferPool(1024, backend)
	cat := New(nil, pool, nil)
	...
	for _, d := range data {
		_, _, err := cat.Insert(&query.InsertStmt{
			Table:   "test_agg",
			Columns: []string{"id", "value"},
			Values:  [][]query.Expression{{&query.NumberLiteral{Value: float64(d.id)}, &query.NumberLiteral{Value: d.value}}},
		}, nil)
		if err != nil {
	...
	// Test aggregate without GROUP BY (uses computeAggregates)
	stmt := &query.SelectStmt{
		Columns: []query.Expression{
			&query.FunctionCall{Name: "COUNT", Args: []query.Expression{&query.StarExpr{}}},
	...
	sumF, sumOk := toFloat64(rows[0][1])
	avgF, avgOk := toFloat64(rows[0][2])
	minF, minOk := toFloat64(rows[0][3])
	maxF, maxOk := toFloat64(rows[0][4])

	if count != 3 {
		t.Errorf("Expected COUNT=3, got %d", count)
	}
	if !sumOk || sumF != 60.0 {
		t.Errorf("Expected SUM=60.0, got %v", rows[0][1])
	}
	if !avgOk || avgF != 20.0 {
		t.Errorf("Expected AVG=20.0, got %v", rows[0][2])
	}
	if !minOk || minF != 10.0 {
		t.Errorf("Expected MIN=10.0, got %v", rows[0][3])
	}
	if !maxOk || maxF != 30.0 {
		t.Errorf("Expected MAX=30.0, got %v", rows[0][4])
	}

	t.Logf("Aggregate result: cols=%v, rows=%v", cols, rows)
}

// TestComputeAggregatesWithWhereCoverage tests computeAggregates with WHERE clause
func TestComputeAggregatesWithWhereCoverage(t *testing.T) {
	backend := storage.NewMemory()
	pool := storage.NewBufferPool(1024, backend)
	cat := New(nil, pool, nil)
	...
	cols, rows, err := cat.Select(stmt, nil)
	if err != nil {
		t.Fatalf("Aggregate with WHERE failed: %v", err)
	}
	...
	count := rows[0][0].(int64)
	sum := rows[0][1].(float64)

	if count != 3 { // id > 2 means rows 3, 4, 5
		t.Errorf("Expected COUNT=3, got %d", count)
	}
	if sum != 120.0 { // 30 + 40 + 50 = 120
		t.Errorf("Expected SUM=120.0, got %f", sum)
	}

	t.Logf("Aggregate with WHERE result: cols=%v, rows=%v", cols, rows)
}
