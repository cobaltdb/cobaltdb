...
// ==============================================================
// computeAggregates is dead code - verify it exists but is never called
// ==============================================================

func TestComputeAggregates_IsDeadCode(t *testing.T) {
	// computeAggregates is defined but never called from the engine
	// This test verifies it can still be called directly
	...
	table := cat.tables["test_agg"]

	// Test COUNT(*)
	selectCols := []selectColInfo{
		{isAggregate: true, aggregateType: "COUNT", aggregateCol: "*", name: "COUNT(*)"},
	}
	cols, rows, err := cat.computeAggregates(table, &query.SelectStmt{
		From: &query.TableRef{Name: "test_agg"},
	}, nil, selectCols, []string{"COUNT(*)"})
	if err != nil {
		t.Fatalf("computeAggregates COUNT(*): %v", err)
	}
	if len(rows) != 1 || len(cols) != 1 {
		t.Fatalf("Expected 1 row, 1 col; got %d rows, %d cols", len(rows), len(cols))
	}
	...
	// Test SUM(val)
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "SUM", aggregateCol: "val", name: "SUM(val)"},
	}
	_, rows, err = cat.computeAggregates(table, &query.SelectStmt{
		From: &query.TableRef{Name: "test_agg"},
	}, nil, selectCols, []string{"SUM(val)"})
	if err != nil {
		t.Fatalf("computeAggregates SUM: %v", err)
	}
	...
	// Test AVG(val)
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "AVG", aggregateCol: "val", name: "AVG(val)"},
	}
	_, rows, err = cat.computeAggregates(table, &query.SelectStmt{
		From: &query.TableRef{Name: "test_agg"},
	}, nil, selectCols, []string{"AVG(val)"})
	if err != nil {
		t.Fatalf("computeAggregates AVG: %v", err)
	}
	...
	// Test MIN(val)
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "MIN", aggregateCol: "val", name: "MIN(val)"},
	}
	_, rows, err = cat.computeAggregates(table, &query.SelectStmt{
		From: &query.TableRef{Name: "test_agg"},
	}, nil, selectCols, []string{"MIN(val)"})
	if err != nil {
		t.Fatalf("computeAggregates MIN: %v", err)
	}
	...
	// Test MAX(val)
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "MAX", aggregateCol: "val", name: "MAX(val)"},
	}
	_, rows, err = cat.computeAggregates(table, &query.SelectStmt{
		From: &query.TableRef{Name: "test_agg"},
	}, nil, selectCols, []string{"MAX(val)"})
	if err != nil {
		t.Fatalf("computeAggregates MAX: %v", err)
	}
	if fmt.Sprintf("%v", rows[0][0]) != "50" {
		t.Errorf("Expected MAX=50, got %v", rows[0][0])
	}

	// Test GROUP_CONCAT(val)
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "GROUP_CONCAT", aggregateCol: "val", name: "GROUP_CONCAT(val)"},
	}
	_, rows, err = cat.computeAggregates(table, &query.SelectStmt{
		From: &query.TableRef{Name: "test_agg"},
	}, nil, selectCols, []string{"GROUP_CONCAT(val)"})
	if err != nil {
		t.Fatalf("computeAggregates GROUP_CONCAT: %v", err)
	}
	// Verify it's a comma-separated string
	if rows[0][0] == nil {
		t.Error("Expected non-nil GROUP_CONCAT")
	}
	gc := fmt.Sprintf("%v", rows[0][0])
	if !strings.Contains(gc, "10") || !strings.Contains(gc, "50") {
		t.Errorf("GROUP_CONCAT should contain 10 and 50, got: %v", gc)
	}

	// Test on non-existent table tree
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "COUNT", aggregateCol: "*", name: "COUNT(*)"},
	}
	_, rows, err = cat.computeAggregates(table, &query.SelectStmt{
	...
	if err != nil {
		t.Fatalf("computeAggregates nonexistent: %v", err)
	}
	if fmt.Sprintf("%v", rows[0][0]) != "0" {
		t.Errorf("Expected COUNT(*)=0 for nonexistent table tree, got %v", rows[0][0])
	}
	...
	if err != nil {
		t.Fatalf("computeAggregates with WHERE: %v", err)
	}
	if fmt.Sprintf("%v", rows[0][0]) != "2" {
		t.Errorf("Expected COUNT(*)=2 for val>30, got %v", rows[0][0])
	}

	// Test COUNT(DISTINCT val)
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "COUNT", aggregateCol: "val", isDistinct: true, name: "COUNT(DISTINCT val)"},
	}
	_, rows, err = cat.computeAggregates(table, &query.SelectStmt{
		From: &query.TableRef{Name: "test_agg"},
	}, nil, selectCols, []string{"COUNT(DISTINCT val)"})
	if err != nil {
		t.Fatalf("computeAggregates COUNT DISTINCT: %v", err)
	}
	if fmt.Sprintf("%v", rows[0][0]) != "5" {
		t.Errorf("Expected COUNT(DISTINCT val)=5, got %v", rows[0][0])
	}
}

// ==============================================================
// computeAggregateResult additional branches
// ==============================================================

func TestComputeAggregateResult_EmptyValues(t *testing.T) {
	backend := storage.NewMemory()
	pool := storage.NewBufferPool(1024, backend)
	defer pool.Close()
	...
	_, rows, err := cat.computeAggregateResult(selectCols, []string{"SUM(val)"}, 0, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult SUM empty: %v", err)
	}
	if rows[0][0] != nil {
		t.Errorf("Expected nil for SUM of empty, got %v", rows[0][0])
	}
	...
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"AVG(val)"}, 0, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult AVG empty: %v", err)
	}
	if rows[0][0] != nil {
		t.Errorf("Expected nil for AVG of empty, got %v", rows[0][0])
	}
	...
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"MIN(val)"}, 0, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult MIN empty: %v", err)
	}
	if rows[0][0] != nil {
		t.Errorf("Expected nil for MIN of empty, got %v", rows[0][0])
	}
	...
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"MAX(val)"}, 0, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult MAX empty: %v", err)
	}
	if rows[0][0] != nil {
		t.Errorf("Expected nil for MAX of empty, got %v", rows[0][0])
	}

	// GROUP_CONCAT with no data
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "GROUP_CONCAT", aggregateCol: "val", name: "GROUP_CONCAT(val)"},
	}
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"GROUP_CONCAT(val)"}, 0, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult GROUP_CONCAT empty: %v", err)
	}
	if rows[0][0] != nil {
		t.Errorf("Expected nil for GROUP_CONCAT of empty, got %v", rows[0][0])
	}
	...
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"COUNT(*)"}, 0, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult COUNT empty: %v", err)
	}
	if fmt.Sprintf("%v", rows[0][0]) != "0" {
		t.Errorf("Expected COUNT(*)=0, got %v", rows[0][0])
	}

	// COUNT(col) with empty values list
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "COUNT", aggregateCol: "val", name: "COUNT(val)"},
	}
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"COUNT(val)"}, 0, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult COUNT(col) empty: %v", err)
	}
	if fmt.Sprintf("%v", rows[0][0]) != "0" {
		t.Errorf("Expected COUNT(val)=0, got %v", rows[0][0])
	}

	// SUM with all-nil values
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "SUM", aggregateCol: "val", name: "SUM(val)"},
	}
	aggValues := [][]interface{}{{nil, nil, nil}}
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"SUM(val)"}, 3, aggValues)
	if err != nil {
		t.Fatalf("computeAggregateResult SUM all nil: %v", err)
	}
	if rows[0][0] != nil {
		t.Errorf("Expected nil for SUM of all nil, got %v", rows[0][0])
	}

	// GROUP_CONCAT with all-nil values
	selectCols = []selectColInfo{
		{isAggregate: true, aggregateType: "GROUP_CONCAT", aggregateCol: "val", name: "GROUP_CONCAT(val)"},
	}
	aggValues = [][]interface{}{{nil, nil}}
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"GROUP_CONCAT(val)"}, 2, aggValues)
	if err != nil {
		t.Fatalf("computeAggregateResult GROUP_CONCAT all nil: %v", err)
	}
	if rows[0][0] != nil {
		t.Errorf("Expected nil for GROUP_CONCAT of all nil, got %v", rows[0][0])
	}

	// Non-aggregate column
	selectCols = []selectColInfo{
		{isAggregate: false, name: "id"},
	}
	_, rows, err = cat.computeAggregateResult(selectCols, []string{"id"}, 1, nil)
	if err != nil {
		t.Fatalf("computeAggregateResult non-aggregate: %v", err)
	}
	// Non-aggregate columns remain nil in result
	if rows[0][0] != nil {
		t.Errorf("Expected nil for non-aggregate column, got %v", rows[0][0])
	}
}
